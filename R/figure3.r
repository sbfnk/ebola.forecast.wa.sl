...
figure3 <- function(max_horizon=5)
{
    ...
    p_calib <- ggplot(scores[["Calibration"]], aes(x=horizon, y=mean, color=model)) +
        geom_line() +
        geom_point() +
    ...
    p_sharpness <- ggplot(scores[["Sharpness"]], aes(x=horizon, y=mean, color=model)) +
        geom_line() +
        geom_point() +
        scale_color_brewer(palette="Set1") +
        ylab("Sharpness") + xlab("Horizon (weeks)")

    p_bias <- ggplot(scores[["Bias"]], aes(x=horizon, y=mean, color=model)) +
        geom_line() +
        geom_point() +
        scale_color_brewer(palette="Set1") +
        ylim(c(-1, 1)) +
    ...
    p_rps <- ggplot(scores[["RPS"]], aes(x=horizon, y=mean, color=model)) +
        geom_line() +
        geom_point() +
        scale_color_brewer(palette="Set1") +
        ylab("RPS") + xlab("Horizon (weeks)")

    p_dss <- ggplot(scores[["DSS"]], aes(x=horizon, y=mean, color=model)) +
        geom_line() +
        geom_point() +
        scale_color_brewer(palette="Set1") +
    ...
    p_logs <- ggplot(scores[["logS"]] %>% filter(is.finite(mean)),
                     aes(x=horizon, y=mean, color=model)) +
        geom_line() +
        geom_point() +
        scale_color_brewer(palette="Set1") +
